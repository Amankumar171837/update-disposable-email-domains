...
class disposableHostGenerator():
    ...
    def generate(self):
        ...
        """
        # Load existing domains first to preserve them
        self.read_files()
        
        # Merge old domains into current set to prevent deletion
        self.domains.update(self.old_domains)
        self.sha1.update(self.old_sha1)
        ...
        for domain in skip:
            try:
                if domain not in self.old_domains:
        ...
        if self.options.get('dns_verify'):
            # verify only new domains - preserve old ones
            domains_to_verify = [d for d in self.domains if d not in self.old_domains]
